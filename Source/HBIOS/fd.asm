...
; *** DIDE/N8/ZETA V2 ***
;
#IF ((FDMODE == FDMODE_DIDE) | (FDMODE == FDMODE_N8) | (FDMODE == FDMODE_ZETA2) | (FDMODE == FDMODE_SMB_WD))
#IF (FDSPECIAL)
DOR_INIT        .EQU    10001100B       ; Special mode, soft reset inactive, dma enabled
#ELSE
...
#IF ((FDMODE == FDMODE_DIDE) | (FDMODE == FDMODE_N8) | (FDMODE == FDMODE_ZETA2) | (FDMODE == FDMODE_SMB_WD))
#IF (FDSPECIAL)
        ; no motor support in special mode
        RET
#ELSE
...
;	LD	A,(CPUMHZ + 3) / 4
	...
	;INC	A			; MAKE SURE RESULT IS AT LEAST 1
	LD	(FCD_TO),A		; INIT TIMEOUT COUNTER
FXFT1	LD	C,0			; OUTER LOOP TIMEOUT COUNTER
FXFT2:	LD	B,0			; SETUP FOR 256 ITERATIONS
FXFT3:	IN	A,(FDC_MSR)		; GET MSR
	CP	0F0H			; WE WANT RQM=1,DIO=1,NDM=1,BUSY=1 (READY TO RECEIVE A BYTE W/ EXEC ACTIVE)
	JR	Z,FXFT4			; GOT IT, DO BYTE READ
...
FXFTW:  OUT	(FDC_DATA),A
	...
	LD	A,D
	OR	E
	JR	NZ,FXFT2		; IF NOT ZERO, REPEAT LOOP
	JR	FXR_END			; CLEAN EXIT

FXFT5:					; OUTER LOOP, REALLY ONLY HAPPENS WHEN WAITING FOR FIRST BYTE OR ABORTED
	CP	0C0H			; IF RQM=1, DIO=1, NDM=0 (EXECUTION ABORTED)
	JR	Z,FXR_ABORT		; BAIL OUT TO ERR ROUTINE, FIX: GO TO SPECIFIC ROUTINE FOR THIS???
	DEC	C
	JR	NZ,FXFT2		; IF NOT ZERO, LOOP SOME MORE
	LD	A,(FCD_TO)
	DEC	A
	LD	(FCD_TO),A
	JR	NZ,FXFT1
